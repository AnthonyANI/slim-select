    ...
    return t.dataset.id = e.id, t.id = this.settings.id + "-" + e.id, t.classList.add(this.classes.option), t.setAttribute("role", "option"), e.class && e.class.split(" ").forEach((s) => {
      ...
      if (this.settings.isMultiple) {
        ...
        if (s.shiftKey && this.lastSelectedOption) {
          ...
          if (m >= 0 && v >= 0) {
            const w = Math.min(m, v), S = Math.max(m, v), y = d.slice(w, S + 1).filter((A) => !c.find((x) => x.id === A.id));
            c.length + y.length <= this.settings.maxSelected ? r = c.concat(y) : r = c;
          } else
            r = c;
        } else a ? (h ? r = c.filter((d) => d.id !== n) : r = c.concat(e), this.lastSelectedOption = e) : (h ? r = c.filter((d) => d.id !== n) : r = c.concat(e), this.lastSelectedOption = e);
      }
  ...
  setupLabelHandlers() {
    ...
    Array.from(new Set(e)).forEach((l) => {
      if (l.__slimSelectLabelHandler)
        return;
      const n = (a) => {
        const o = a.target, c = E(o, this.select.dataset.id);
...
class I {
  ...
  constructor(e) {
    ...
    this.selectEl.dataset.ssid && this.destroy(), this.settings = new M(e.settings), this.cssClasses = new T(e.cssClasses);
